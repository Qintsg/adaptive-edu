...
class MEFKTServiceTests(SimpleTestCase):
    ...
    def test_mefkt_predictor_should_load_checkpoint_and_return_predictions(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            torch.save(
                {"state_dict": model.state_dict(), "metadata": metadata}, model_path
            )
    ...
    def test_mefkt_predictor_should_support_question_online_runtime(self):
        ...
        from ai_services.services.mefkt.inference import (
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            runtime_bundle = CourseQuestionRuntimeBundle(
                ...
                node_feature_matrix=torch.full(
                    (2, feature_dim), 0.25, dtype=torch.float32
                ),
                relation_stats_matrix=torch.full(
                    (2, relation_dim), 0.5, dtype=torch.float32
                ),
