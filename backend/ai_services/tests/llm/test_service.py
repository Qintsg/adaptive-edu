...
class LLMProviderConfigTests(SimpleTestCase):
    ...
    def test_llm_service_should_attach_https_proxy_to_chat_client(
...
class LLMServiceRoutingTests(SimpleTestCase):
    ...
    def test_call_with_fallback_should_skip_agent_for_profile_analysis(self):
        ...
        service._get_agent_service = Mock(
            return_value=Mock(
                is_available=True,
                invoke_json=Mock(return_value={"summary": "agent结果"}),
            )
        )
...
class LLMServiceLatencyPolicyTests(SimpleTestCase):
    ...
    def test_chat_policy_should_fast_fail_like_other_interactive_routes(self):
        ...
        self.assertEqual(
            policy.request_timeout_seconds, service.GATEWAY_SAFE_TIMEOUT_SECONDS
        )
    ...
    def test_stream_text_with_fallback_should_yield_model_chunks(self):
        ...
        chunks = list(
            service.stream_text_with_fallback(
                prompt="请回答",
                call_type="chat",
    ...
    def test_stream_text_with_fallback_should_yield_fallback_when_stream_fails_early(
        self,
    ):
        """流式调用首块前失败时应输出降级文本。"""
        service = self._build_service()
...
class LangChainAgentProxyTests(SimpleTestCase):
    ...
    def test_agent_graphrag_tool_should_call_public_payload_builder(self):
        ...
        with (
            ...
            payload = json.loads(
                tools[1](course_id=7, query="解释先修关系", point_id=3, limit=5)
            )
...
class FacadeGraphRAGLLMTests(SimpleTestCase):
    ...
    def test_facade_graphrag_llm_should_accept_v2_message_lists(self):
        ...
        call_with_fallback = Mock(
            return_value={"content": "MATCH (n) RETURN n LIMIT 5"}
        )
