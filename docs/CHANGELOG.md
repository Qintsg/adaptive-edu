...
- 修复 OpenAPI 真实 HTTP 巡检暴露的 6 个 5xx：画像刷新课程外键错误、教师班级学生画像旧字段访问、教师资源 multipart 知识点单值解析、能力重测与知识图谱问答装饰器互调异常均改为稳定的业务响应。
- 能力评测入口统一校验可选课程 ID，非数字或不存在课程返回 400；教师资源创建支持 multipart 重复知识点字段，资源变更后的课程 RAG 索引刷新改为提交后后台触发，避免资源接口被索引重建阻塞。
- 教师班级学生画像列表支持传入 `course_id` 明确多课程班级的画像上下文；资源更新显式传空知识点列表时可清空既有关联。
- 资源创建和更新会先校验知识点 ID 格式及课程归属，非法知识点入参返回 400，不再透传到多对多关联写入形成 500。
- 学生知识点详情默认不再构造 GraphRAG 证据，仅在显式 `include_graph_rag=true` 时启用增强；前端知识点详情 API 同步默认传 `false`，避免详情页被慢链路阻塞。
- 学习节点 AI 资源接口默认使用课程内资源快速推荐，不触发 LLM 或外部搜索；底层 RAG 服务保留显式 `use_llm=True` 的智能推荐能力。
- `docs/API端点测试报告.md` 已更新为最终巡检结果：225 个 OpenAPI operation 全部命中文档化响应，0 个 5xx，0 个请求异常，0 个未文档化状态。

### Docs / API — OpenAPI 端点真实 HTTP 巡检

- 新增 `docs/API端点测试报告.md`，基于本地后端、`pg-bootstrap` 样例库和学生/教师/管理员账号，对 `docs/api.yaml` 中 225 个 OpenAPI operation 逐项发起 HTTP 请求并记录状态、角色、参数、耗时和响应摘要。
- 根据巡检结果补齐 OpenAPI 契约中的常见真实响应状态，包括注册 `201`、刷新令牌 `401`、参数校验 `400`、权限不足 `403` 和资源不存在 `404`。
- `README.md`、`docs/README.md` 与 `docs/使用说明.md` 同步新增 API 端点测试报告入口，便于联调和验收时对照接口运行态表现。
